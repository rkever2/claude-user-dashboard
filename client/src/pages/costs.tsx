        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {isLoading ? (
            ...
            <>
              ...
              <StatCard
                label="This Month"
                value={formatCurrency(data.thisMonthCostUSD)}
                sub="estimated at API rates"
              />
              <StatCard
                label="Daily Average"
                value={
                  data.daily.length > 0
                    ? formatCurrency(data.totalEstimatedCostUSD / data.daily.length)
                    : "$0.00"
                }
              />
            </>
          ...
          <DataTable
            columns={[
              ...
              ...Object.keys(data?.daily[0]?.costByModel || {}).map((model) => ({
                ...
                render: (row: NonNullable<typeof data>["daily"][0]) =>
                  formatCurrency(row.costByModel[model] || 0),
              })),
              {
                key: "total",
                header: "Total",
