			...
			<div className="p-8">
				...
				<div className="border border-border">
					<div className="px-6 pt-5 pb-3">
						<h3 className="text-[11px] uppercase tracking-[0.05em] font-semibold text-muted">All projects</h3>
					</div>
					<DataTable
						columns={[
							{
								key: "name",
								header: "Project",
								render: (row: ProjectStats) => <span className="font-medium">{row.displayName}</span>,
							},
							...
							{ key: "messages", header: "Messages", align: "right", render: (row) => formatNumber(row.totalMessages) },
							{
								...
								header: "Top Branches",
								render: (row) => (
									<span className="text-xs font-mono text-muted">{row.branches.slice(0, 3).join(", ")}</span>
								),
							},
							{
								key: "first",
